...
#[repr(transparent)]
pub struct WasmArray(pub(crate) SharedCollection<Vec<JsValue>, ArrayRef>);

#[wasm_bindgen(js_class = "Array")]
impl WasmArray {
    ...
    pub fn new(items: Option<Vec<JsValue>>) -> Self {
        WasmArray(SharedCollection::prelim(items.unwrap_or_default()))
    }
    ...
    pub fn quote(
        ...
        match &self.0 {
            ...
            SharedCollection::Integrated(c) => c.doc.transact(None, |tx| {
                ...
                Ok(WasmWeakLink::from_prelim(quote, c.doc.clone()))
            }),
        }
    }
...
pub(crate) trait ArrayExt: YArray + SharedRef {
    fn insert_at<I>(&self, txn: &mut YTransaction<crate::WasmDoc>, index: u32, src: I) -> Result<()>
    where
...
#[wasm_bindgen(js_name = "ArrayEvent")]
...
impl WasmArrayEvent {
    ...
    pub fn target(&mut self) -> JsValue {
        ...
        let text_ref = WasmArray(SharedCollection::Integrated(Integrated {
            hook,
            doc: self.doc.clone(),
        }));
        text_ref.into()
