...
pub struct WasmText(pub(crate) SharedCollection<String, TextRef>);

#[wasm_bindgen(js_class = "Text")]
impl WasmText {
    ...
    pub fn new(init: Option<String>) -> Self {
        WasmText(SharedCollection::prelim(init.unwrap_or_default()))
    }
    ...
    pub fn to_json(&self) -> crate::Result<JsValue> {
        match &self.0 {
            ...
            SharedCollection::Integrated(c) => c.transact(|c, txn| Ok(c.get_string(txn).into())),
        }
    }
...
#[wasm_bindgen(js_name = "TextEvent")]
...
impl WasmTextEvent {
    ...
    pub fn target(&mut self) -> JsValue {
        ...
        let text_ref = WasmText(SharedCollection::Integrated(Integrated {
            hook,
            doc: self.doc.clone(),
        }));
        text_ref.into()
