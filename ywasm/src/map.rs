...
/// shared data types.
///
/// In terms of conflict resolution, [WasmMap] uses logical last-write-wins principle, meaning the past
...
pub struct WasmMap(pub(crate) SharedCollection<HashMap<String, JsValue>, MapRef>);
...
impl WasmMap {
    ...
    pub fn new(init: Option<js_sys::Object>) -> Self {
        ...
        WasmMap(SharedCollection::prelim(map))
    }
    ...
    pub fn link(&self, key: &str) -> crate::Result<JsValue> {
        match &self.0 {
            ...
            SharedCollection::Integrated(c) => c.doc.transact(None, |tx| {
                ...
                match link {
                    Some(link) => Ok(WasmWeakLink::from_prelim(link, c.doc.clone()).into()),
...
impl WasmMapEvent {
    ...
    pub fn target(&mut self) -> JsValue {
        ...
        let text_ref = WasmMap(SharedCollection::Integrated(Integrated {
            hook,
            doc: self.doc.clone(),
        }));
        text_ref.into()
