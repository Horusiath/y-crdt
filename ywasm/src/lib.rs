...
pub fn encode_state_from_snapshot_v1(
    #[wasm_bindgen(unchecked_param_type = "Doc")] doc: JsValue,
    snapshot: JsValue,
) -> Result<Vec<u8>> {
...
pub fn encode_state_from_snapshot_v2(
    #[wasm_bindgen(unchecked_param_type = "Doc")] doc: JsValue,
    snapshot: JsValue,
) -> Result<Vec<u8>> {
    let snapshot: Snapshot = snapshot
...
pub fn create_sticky_index_from_type(ytype: JsValue, index: u32, assoc: i32) -> Result<JsValue> {
    if let Ok(shared) = Shared::from_ref(&ytype) {
        ...
        let index = crate::Doc::transact(doc, JsValue::UNDEFINED, |tx| {
            ...
            Ok(StickyIndex::at(doc, ptr, index, assoc))
        })?;
...
pub fn create_offset_from_sticky_index(
    rpos: &JsValue,
    #[wasm_bindgen(unchecked_param_type = "Doc")] doc: JsValue,
) -> Result<JsValue> {
    #[derive(Serialize)]
