...
#[wasm_bindgen(js_name = "XmlText")]
pub struct WasmXmlText(pub(crate) SharedCollection<PrelimXmlText, XmlTextRef>);

#[wasm_bindgen(js_class = "XmlText")]
impl WasmXmlText {
    #[wasm_bindgen(constructor)]
    pub fn new(text: Option<String>, attributes: Option<JsValue>) -> crate::Result<WasmXmlText> {
        let attributes = match attributes {
            Some(attributes) => XmlAttrs::parse_attrs_any(attributes)?,
            None => Attrs::default(),
        };
        Ok(WasmXmlText(SharedCollection::prelim(PrelimXmlText {
        ...
        match &mut self.0 {
            ...
            SharedCollection::Integrated(c) => c.transact(|c, txn| {
                if attributes.is_undefined() || attributes.is_null() {
                    ...
                    Ok(())
                } else if let Some(attrs) = WasmText::parse_fmt(attributes) {
    ...
    pub fn format(&self, index: u32, length: u32, attributes: JsValue) -> crate::Result<()> {
        let attrs = match WasmText::parse_fmt(attributes) {
            Some(attrs) => attrs,
        ...
        match &self.0 {
            ...
            SharedCollection::Integrated(c) => {
                ...
                c.transact(|c, txn| {
                    ...
                    Ok(WasmWeakLink::from_prelim(quote, doc))
                })
            }
        }
    }
            ...
            SharedCollection::Integrated(c) => {
                ...
                c.doc.transact(None, |tx| {
                    ...
                    target.observe_with(abi, move |tx, e| {
                        ...
                        let e = WasmXmlTextEvent::new(e, &doc, &origin);
...
#[wasm_bindgen(js_name = "XmlTextEvent")]
...
#[wasm_bindgen(js_class = "XmlTextEvent")]
impl WasmXmlTextEvent {
    pub(crate) fn new<'doc>(event: &XmlTextEvent, doc: &crate::WasmDoc, origin: &JsValue) -> Self {
