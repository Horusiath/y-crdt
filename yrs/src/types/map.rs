...
use crate::types::{
    event_keys, Branch, BranchRef, Entries, EntryChange, Event, Observers, Path, TypePtr, Value,
    TYPE_REFS_MAP,
};
use crate::*;
use lib0::any::Any;
...
impl Map {
        ...
        F: Fn(&Transaction, &MapEvent) -> () + 'static,
    {
        ...
        if let Observers::Map(eh) = branch.observers.get_or_insert_with(Observers::map) {
            ...
            panic!("Observed collection is of different type")
        }
    }
    ...
    pub fn observe_deep<F>(&self, f: F) -> Subscription<Event>
    where
        F: Fn(&Transaction, &Event) -> () + 'static,
    {
        self.0.borrow_mut().observe_deep(f)
    }

    pub fn unobserve_deep(&self, subscription_id: SubscriptionId) {
        let mut branch = self.0.borrow_mut();
        if let Some(eh) = branch.deep_observers.as_mut() {
            eh.unsubscribe(subscription_id);
        }
    }
}
...
impl<T> Default for PrelimMap<T> {
    fn default() -> Self {
...
pub struct MapEvent {
    target: Map,
    pub(crate) current_target: BranchRef,
...
mod test {
    ...
    use crate::types::{EntryChange, Map, PathSegment, Value};
    ...
    #[test]
    fn deep_observe() {
        fn as_map(o: Option<Value>) -> Map {
            o.unwrap().try_into().unwrap()
        }
        ...
        let mut txn = d1.transact();
        m1.insert(&mut txn, "map", PrelimMap::<u32>::default());
        let m11: Map = m1.get(&txn, "map").unwrap().try_into().unwrap();

        let dmapid = Rc::new(RefCell::new(None));
        let mut dmapid_cp = dmapid.clone();
        let calls = Cell::new(0);
        let calls_cp = calls.clone();
        let _sub = m1.observe_deep(move |txn, e| {
            calls_cp.set(calls_cp.get() + 1);
            let path = e.path(txn);
            assert_eq!(path, vec![PathSegment::Key("map".into())]);
            // t.assert(event.keysChanged.has('deepmap'))
            let inner: &MapEvent = e.try_into().unwrap();
            dmapid_cp.borrow_mut().insert(inner.target().clone());
        });
        ...
        let d3 = Doc::with_client_id(3);
        let m3 = {
            let mut txn = d3.transact();
            txn.get_map("map")
        };
        exchange_updates(&[&d1, &d2, &d3]);
        let m31 = as_map(m3.get(&d3.transact(), "map"));
        m31.insert(&mut d3.transact(), "deepmap", PrelimMap::<u32>::default());
        exchange_updates(&[&d1, &d2, &d3]);

        let m21 = as_map(m2.get(&d2.transact(), "map"));
        m21.insert(&mut d2.transact(), "deepmap", PrelimMap::<u32>::default());
        exchange_updates(&[&d1, &d2, &d3]);

        let dmap1 = as_map(m11.get(&d1.transact(), "deepmap"));
        let dmap2 = as_map(m21.get(&d2.transact(), "deepmap"));
        let dmap3 = as_map(m31.get(&d3.transact(), "deepmap"));

        assert!(calls.get() > 0);
        let dmap1_id = &dmap1.as_ref().borrow().ptr;
        let dmap2_id = &dmap2.as_ref().borrow().ptr;
        let dmap3_id = &dmap3.as_ref().borrow().ptr;
        assert_eq!(dmap1_id, dmap2_id);
        assert_eq!(dmap1_id, dmap3_id);
        let dmap_id = dmapid.borrow_mut().take().unwrap().0.borrow().ptr.clone();
        assert_eq!(dmap1_id, &dmap_id);
    }
}
